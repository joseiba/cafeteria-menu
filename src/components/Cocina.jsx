...
const Cocina = ({}) => {
    ...
    useEffect(() => {
        ...
        .then(function(res) {
          ...
          let p = res.data
          setPedidos(p.filter((p) => p.estado == 0))        
          console.log(pedidos)
        })
    ...
    const onListo = (pedido) =>{
      client.put("/pedidos/" + pedido.id + "/",  {
            estado: 1,
        },
      {
        headers: {
        ...
        <div className=''>
          ...
          <tbody>
                {pedidos.map(pedidos =>
                  ...
                  <td>
                    <div>
                        <button className='btn btn-primary' type='button' onClick={() => onListo(pedidos)}>Listo</button>
                    </div>
                  </td>
                </tr>
            ...
            <h1 className='form-label'>No se tiene ningun pedido</h1>
          )
          }       
        </div>
